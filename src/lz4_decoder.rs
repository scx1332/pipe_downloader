...
mod test {
    ...
    fn finish_decode<R: Read>(decoder: Lz4Decoder<R>) {
    ...
    fn test_decoder_empty() {
        ...
        let mut decoder = Lz4Decoder::new(Cursor::new(buffer)).unwrap();
    ...
    fn test_decoder_random() {
        ...
        let mut decoder = Lz4Decoder::new(Cursor::new(encoded)).unwrap();
        let mut actual = Vec::new();
        loop {
    ...
    fn test_retry_read() {
        ...
        let mut decoder =
            Lz4Decoder::new(ErrorWrapper::new(rnd.clone(), Cursor::new(encoded))).unwrap();
        let mut actual = Vec::new();
        loop {
            let mut buffer = [0; BUFFER_SIZE];
    ...
    fn test_decoder_send() {
        ...
        let dec = Lz4Decoder::new(Cursor::new(Vec::new())).unwrap();
        check_send(&dec);
    }
}
